...
### Security
- **`pypdf` 6.15.0 → 6.16.1** — PYSEC-2026-3910, PYSEC-2026-3911, and PYSEC-2026-3913, all with 6.16.1 as the published fix (3913 is already fixed in 6.16.0). `pip-audit` in the Security Scan job started failing on unchanged MCP work once the advisories landed. `pypdf` is used by `create_songbook.py` and `prepare_singles.py`; 6.16.1 still requires Python >=3.9, so the 3.11 floor is unaffected.
