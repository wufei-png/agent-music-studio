...
### Added
- **Agent-portable music workflows and an isolated Codex plugin package** — the complete Claude Code plugin remains intact while a first portable slice exposes `resume`, `lyric-writer`, and `suno-engineer` through standard Agent Skills. The generic adapters avoid host-specific invocation and package-layout assumptions. The Codex build carries an explicit, isolated runtime bootstrap, host-neutral MCP launch metadata, a separate canonical runtime version for upstream migrations, an expected-skill inventory for installation health checks, and self-contained skill resources.
